...
def get_posts(status: Optional[str] = None):
    ...
    snaps = query.order_by("created_at", direction="DESCENDING").limit(50).get()
...
def admin_queue(current_user: Optional[dict] = Depends(get_current_user)):
    db = get_db_client()
    snaps = db.collection("posts").where("status", "==", "human_review_required").order_by("created_at", direction="DESCENDING").get()
    results = []
    for snap in snaps:
        post = snap.to_dict()
        report = None
...
def get_posts_enriched(statuses: str = "verified,debunked", limit: int = 50):
    ...
    snaps = query.order_by("created_at", direction="DESCENDING").limit(limit).get()
    ...
    for snap in snaps:
        ...
        elif post.get("status") == "generating_video":
            render_job = {
                "post_id": post.get("id"),
                "status": "processing"
            }
...
def get_render_jobs():
    db = get_db_client()
    snaps = db.collection("posts").where("status", "in", ["generating_video", "published"]).order_by("created_at", direction="DESCENDING").get()
    jobs = []
    for snap in snaps:
        post = snap.to_dict()
