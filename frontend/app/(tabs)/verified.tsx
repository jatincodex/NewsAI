...
export default function VerifiedScreen() {
  ...
  const load = useCallback(async () => {
    try {
      const verified = await api.posts("verified");
      const debunked = await api.posts("debunked");
      const combined = [...verified, ...debunked];
      const enriched: Row[] = [];
      for (const p of combined) {
        const full = await api.post(p.id);
        enriched.push({ post: full.post, report: full.report, render_job: full.render_job });
      }
